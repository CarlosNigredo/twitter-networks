//ESTADISTICAS CORRECTAS EN CONSOLA 
d3.json("data/HoyNoCircula/hoy_no_circula_Apr052016_filtro.json", function(data) {
    d3.json("data/ListasActores/hoy_no_circula_Apr022016_clas.json", function(cuentas) {

    links = []

    var retuits=0, replies=0, mentions=0;
    ...
    // console.log("rt:"+retuits+"m:"+mentions+"rp:"+replies);
    ...
    var topNodes = [];//30 primeros nodos
    ...
    //Lee el json de clasificación de cuentas y asigna una clase a cada nodo
        ...
        nodes.forEach(function(d) {
            ...
            for (var i = 0; i < cuentas.length; i++) {
                if (str.toLowerCase() == cuentas[i].name) {
                    d.type = cuentas[i].score;
                }
            }
        ...
        console.log("Retuits: "+numRT+"("+porcentaje(numRT,totalInteractions.length)+"%)"+", Respuestas: "+numRP+"("+porcentaje(numRP,totalInteractions.length)+"%)"+", Menciones: "+numMen+"("+porcentaje(numMen,totalInteractions.length)+"%)");
        console.log(" ");
        ...
        console.log(" ");
        console.log("INTRA-ACTORES: "+intraActores.length+"("+porcentaje(intraActores.length,totalInteractions.length)+"% del total de interacciones)");
        ...
        console.log(" ");
        console.log("INTER-ACTORES: "+interActores.length+"("+porcentaje(interActores.length,totalInteractions.length)+"% del total de interacciones)");
        ...
        d3.selectAll("#selectorsNodes label").on("mousedown", function(d) {
        ...
        d3.select("#et").on("click", function(d) {
            if (top10Labels === 0 && howManyLinks===3) {
                // text.transition(350)
                    text.style("opacity", 1);
                top10Labels = 1;
    ...
    d3.select("#tamanio").on("click", function(d) {
            sameSize();
    });

    function porcentaje(d,total){
        return ((d*100)/total).toFixed(2);
    }
});
});